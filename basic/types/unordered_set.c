...
static void collect_unordered_set_data_phase_2(
  void *old_data
);
...
static void collect_unordered_set_data_phase_2(
  ...
  //fprintf(stderr, "collect_unordered_set_data_phase_2 (%p)\n", old_data);
  UNORDERED_SET_DATA *data = DECODE_ADDRESS(*(void **)old_data);
...
static UNORDERED_SET_DATA *collect_unordered_set_data(
  ...
  if (IS_COLLECTED(data)) {
    ...
    if (IS_AN_INVALID_ADDRESS(new_location)) {
      ...
      ((PHASE_2_COLLECTOR *)data)->collector = collect_unordered_set_data_phase_2;
